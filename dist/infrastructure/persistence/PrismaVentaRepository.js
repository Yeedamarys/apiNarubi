...
class PrismaVentaRepository {
    async guardarTransaccional(venta) {
        return await prismaClient_1.prisma.$transaction(async (tx) => {
            ...
            const rawVenta = await tx.venta.create({
                data: {
                    ...
                    fecha: venta.fecha,
                    modalidad_venta: venta.modalidadVenta,
            ...
            const claveSri = venta.claveAccesoSri || Venta_1.Venta.generarClaveAccesoSRI(rawVenta.id);
    ...
    async listar() {
        const raws = await prismaClient_1.prisma.venta.findMany({
            include: {
                cliente: true,
                usuario: true,
                bodega: true,
    ...
    async buscarPorId(id) {
        const raw = await prismaClient_1.prisma.venta.findUnique({
            where: { id },
            include: {
                cliente: true,
                usuario: true,
