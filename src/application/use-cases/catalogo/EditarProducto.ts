  ...
  public async ejecutar(id: number, input: EditarProductoInput): Promise<Producto> {
    ...
    if (input.codigoBarras && input.codigoBarras.trim() !== productoExistente.codigoBarras) {
      ...
      if (duplicado && duplicado.id !== id) {
        throw new AppError(`El código de barras '${input.codigoBarras}' ya pertenece a otro producto.`, 400, 'CODIGO_BARRAS_DUPLICADO');
      }
    }
    ...
    const datosActualizacion: Partial<Producto> = {
      ...
      ...(input.unidadesPorPaquete !== undefined && { unidadesPorPaquete: input.unidadesPorPaquete }),
