...
export class PrismaRecepcionRepository implements RepositorioRecepcionPort {
  public async guardarTransaccional(recepcion: RecepcionMercaderia): Promise<RecepcionMercaderia> {
    return await prisma.$transaction(async (tx) => {
      ...
      return RecepcionMercaderia.crear({
        id: rawCabecera.id,
  ...
  public async listar(): Promise<RecepcionConDetalle[]> {
    const raws = await prisma.recepcion_mercaderia.findMany({
      include: {
        proveedor: true,
        bodega: true,
        usuario: true,
  ...
  public async buscarPorId(id: number): Promise<RecepcionConDetalle | null> {
    const raw = await prisma.recepcion_mercaderia.findUnique({
      where: { id },
      include: {
        proveedor: true,
        bodega: true,
